    ...
    class Server : MarshalByRefObject, BasicOperations
        {
            ...
            private DateTime startTime; //set the start time to the current time
            //when the client asks to start a calculation
            ...
            public bool start(int initValue)
            {
                ...
                //Program.tokenExistance = true; //tokenExistance true
                startTime = DateTime.Now; //the time when the calculation starts
                Console.WriteLine("Start a round of calculation at time {0}", startTime);
                Results.Add(initValue.ToString()); //add initial value to the list
                return true;
            }
